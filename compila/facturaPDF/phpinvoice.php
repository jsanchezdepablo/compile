	...
	public function addItem($item,$description = "",$quantity,$vat,$price,$discount = 0,$total) {
		...
		if($discount !== false) {
			...
			if(is_numeric($discount)) {
				$p['discount']	= $this->currency.' '.number_format($quantity,2,$this->referenceformat[0],$this->referenceformat[1]);
			}
	...
	public function Body() {	
		...
		if($this->items) {
			foreach($this->items as $item) 
			{
				...
				$this->Cell($this->columnSpacing,$cHeight,'',0,0,'L',0);
				$this->Cell($width_other,$cHeight,iconv('UTF-8', 'windows-1252', "%".' '.number_format($item['discount'],0,$this->referenceformat[0],$this->referenceformat[1])),0,0,'C',1);
				if(isset($this->discountField)) 
				{
					$this->Cell($this->columnSpacing,$cHeight,'',0,0,'L',0);
